...
# Function to get clipboard content (prioritized over selection)
...
get_selection() {
  ...
  # First try to get clipboard content
  selected_text=$(get_clipboard_content)

  # If clipboard is empty, try to get selection
  if [ -z "$selected_text" ]; then
    ...
    if is_xwayland; then
      ...
      if command -v xclip >/dev/null; then
        ...
        # If selection is empty and it's Logseq, try forcing a copy
    ...
    # If native Wayland window
      ...
      # If selection is empty, try forcing a copy
      if [ -z "$selected_text" ]; then
        if command -v ydotool >/dev/null; then
          ydotool key ctrl+c
          sleep 0.5
...
# Alternative function that ONLY uses clipboard (no selection fallback)
get_clipboard_only() {
  local clipboard_text=""
...
# Choose which function to use based on your preference
# Option 1: Prioritize clipboard, fallback to selection
selected_text=$(get_selection)

# Option 2: Use ONLY clipboard content (uncomment to use this instead)
# selected_text=$(get_clipboard_only)

# If no text is found after all attempts, exit
if [ -z "$selected_text" ]; then
  notify-send --icon=error "Translation Error" "No text in clipboard or unable to access clipboard content"
  exit 1
fi
...
notify-send --icon=info "Original Text (Clipboard)" "<span foreground='$original_text_color'>$selected_text</span>"
