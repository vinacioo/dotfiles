...
winclass="$(xdotool search --class scratchpad-term)"
if [ -z "$winclass" ]; then
  alacritty --class scratchpad-term -e bash -c "tmux -u"
else
  if [ ! -f /tmp/scratchpad-term ]; then
    touch /tmp/scratchpad-term && xdotool windowunmap "$winclass"
  elif [ -f /tmp/scratchpad-term ]; then
    rm /tmp/scratchpad-term && xdotool windowmap "$winclass"
  fi
fi
